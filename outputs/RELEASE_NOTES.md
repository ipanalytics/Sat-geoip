# sat-geoip Dataset Release

Generated artifacts for the current sat-geoip dataset build.

| Metric | Count |
|---|---:|
| Prefixes | 14259 |
| Announced prefixes | 10687 |
| GeoFeed-only prefixes | 3572 |
| BGP-only prefixes | 9693 |
| Prefixes with PoP assignment | 3532 |
| Active user claims | 10687 |
| Ground station claims | 0 |

## Operators

| Name | Count |
|---|---:|
| `anuvu` | 64 |
| `avanti` | 23 |
| `bentley_walker` | 33 |
| `caprock` | 3 |
| `carnival` | 1 |
| `castor_marine` | 4 |
| `china_satcom` | 84 |
| `esa` | 79 |
| `eutelsat_skylogic` | 276 |
| `gazprom_space_systems` | 45 |
| `gilat_telecom` | 227 |
| `gogo_business_aviation` | 1 |
| `hispasat` | 41 |
| `hughes` | 675 |
| `inmarsat` | 55 |
| `intelsat` | 95 |
| `intelsat_general` | 3 |
| `iridium` | 11 |
| `itc_global` | 8 |
| `kacific` | 26 |
| `kt_sat` | 4 |
| `kuiper` | 2 |
| `kvh` | 61 |
| `marlink` | 29 |
| `nasa_jpl` | 15 |
| `navarino` | 11 |
| `nbn_sky_muster` | 459 |
| `nsslglobal` | 1 |
| `omniaccess` | 13 |
| `oneweb` | 19 |
| `panasonic_avionics` | 13 |
| `rignet` | 1 |
| `rocket_lab` | 2 |
| `royal_caribbean` | 1 |
| `rscc` | 30 |
| `satcom_direct` | 4 |
| `ses_o3b` | 22 |
| `sky_perfect_jsat` | 6 |
| `spacex_infrastructure` | 2 |
| `speedcast` | 108 |
| `starlink` | 5922 |
| `swarm` | 1 |
| `tampnet` | 6 |
| `telesat` | 45 |
| `telespazio` | 13 |
| `thales_avionics` | 1 |
| `thuraya` | 10 |
| `turksat` | 950 |
| `usap` | 19 |
| `viasat` | 4636 |
| `yahsat` | 99 |


## Orbit Classes

| Name | Count |
|---|---:|
| `deep_space` | 15 |
| `geo` | 2727 |
| `geo_mss` | 29 |
| `geo_or_hybrid_satellite` | 5015 |
| `geo_or_multi_orbit` | 95 |
| `hybrid_satellite_offshore` | 6 |
| `leo` | 5959 |
| `meo` | 22 |
| `mixed_satellite` | 391 |


## Quality Flags

| Name | Count |
|---|---:|
| `bgp_announced` | 10687 |
| `bgp_not_announced` | 3572 |
| `geoip_invalid_country_city_pair` | 13 |
| `geoip_valid` | 4566 |
| `origin_asn_expected` | 10687 |
| `pop_missing` | 1034 |
| `pop_present` | 3532 |
| `prefix_only_in_bgp` | 9693 |
| `prefix_only_in_geofeed` | 3572 |
| `ptr_missing` | 3532 |


